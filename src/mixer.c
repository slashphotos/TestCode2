...
void mixTable(void)
{
    static   int16_t LPFmotor[MAX_MOTORS];                                        // LPF
    int16_t  maxMotor;
    uint32_t i;
   ...
   case MULTITYPE_FLYING_WING:
    ...
    for (i = 0; i < NumberOfMotors; i++)
    {
        ...
        if(cfg.esc_gain)                                                  // Idea Bob Doiron: http://diydrones.com/profiles/blogs/arducopter-motor-response-compensation
        {
            motor[i]   += constrain(((motor[i] - LPFmotor[i]) * (int16_t)cfg.esc_gain) >> 4, -500, 500);
            motor[i]    = constrain(motor[i], cfg.esc_min, cfg.esc_max);
            LPFmotor[i] = ((LPFmotor[i] << 1) + LPFmotor[i] + motor[i]) >> 2;
        }
