...
void computeIMU(void)
{
    ...
    curT               = micros();
    ACCDeltaTimeINS    = (float)(curT - prevT) * 0.000001f;
    INVACCDeltaTimeINS = 1.0f / ACCDeltaTimeINS;
    prevT              = curT;
...
static void getEstimatedAttitude(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        DeltGyRad[i]  = (ACCDeltaTimeINS * gyroADC[i]) * 0.01f;               // gyroADC delivered in 100 * rad/s
    ...
    Norm         = 1.0f / Norm;
    rollRAD      =  atan2f(EstG.A[0], EstG.A[2]);
...
void getAltitudePID(void)                                                     // I put this out of getEstimatedAltitude seems logical
{
    ...
    BaroI = (int16_t)(((float)cfg.I8[PIDALT] * vario) * 0.02f);               // BaroI = (int16_t)(((float)cfg.I8[PIDALT] * vario * INVACCDeltaTimeINS) * 0.00006f); // That is actually a "D"
